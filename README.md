# Estudio Barbat — juanbarbat.dev

El sitio de Juan I. Barbat: CV, portfolio y estudio de desarrollo web a medida e
IA aplicada. Un único archivo HTML autocontenido (`index.html`): CSS, JS, la foto
y la fuente van embebidos, sin build ni dependencias.

## Cómo verlo

Abrí `index.html` en el navegador (doble clic o `file://`). No necesita
`npm install` ni servidor.

## Deploy

Subir `index.html` y `.htaccess` a la raíz del hosting (Hostinger / Apache). El
`.htaccess` fuerza no-cache en el HTML y cache largo en los assets con hash.
